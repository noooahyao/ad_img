...
parser.add_argument('--model', type=str, default='../models/vgg_16.ckpt')
...
def Train(input_dir, output_dir):
  ...
  with tf.Graph().as_default():
    ...
    predict = tf.argmax(slim.softmax(logits),1)
    ...
    with tf.Session() as sess:
      ...
      if FineTune:
          exclusion=['%s/fc8'%model_type,'%s/dropout7'%model_type]#去除class类里的描述
      ...
      for time in range(epoch_num):
        ...
        for raw_images, true_labels in load_images_with_true_label(input_dir,train=True):
            ...
            img = preprocess_for_model(raw_images, model_type,train=True)
