...
def non_target_graph(ori_x,x, y, i, x_max, x_min, grad):
  ...
  with slim.arg_scope(inception.inception_v1_arg_scope()):
    logits_inc_v1, end_points_inc_v1 = inception.inception_v1(
      image, num_classes=num_classes, is_training=False, scope='InceptionV1',reuse=True)
  ...
  pred = tf.argmax(end_points_inc_v1['Logits'],1)
...
def img_aug(img):
  ...
  #random_factor = np.random.randint(0, 5) / 10.  # 轚~O彜º佛| 嬾P
  #color_image = ImageEnhance.Color(img).enhance(random_factor)  # 课C录´佛¾佃~O潚~D饱佒~L度
  #random_factor = np.random.randint(0, 5) / 10.  # 轚~O彜º佛| 嬾P
  #brightness_image = ImageEnhance.Brightness(color_image).enhance(random_factor)  # 课C录´佛¾佃~O潚~D人®度
  #random_factor = np.random.randint(0,5) / 10.  # 轚~O彜º佛| 1嬾P
  #contrast_image = ImageEnhance.Contrast(brightness_image).enhance(random_factor)  # 课C录´佛¾佃~O对殾~T度
  #random_factor = np.random.randint(0,5) / 10.  # 轚~O彜º佛| 嬾P
...
def non_target_mi_fgsm_attack(input_dir, output_dir):
  ...
  with tf.Graph().as_default():
    ...
    with slim.arg_scope(inception.inception_v1_arg_scope()):
      logits_inc_v1, end_points_inc_v1 = inception.inception_v1(
        train_img, num_classes=num_classes, is_training=True, scope='InceptionV1')    
    ...
    with tf.Session() as sess:
      ...
      for time in range(epoch_num):
        ...
        s1.save(sess=sess,save_path='./train_model/%s_%s.ckpt'%(model_type,time))
