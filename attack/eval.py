...
def load_images_with_true_label(input_dir,origin_dir):
    ...
    for filename in filename2label.keys():
        image = imresize(imread(os.path.join(input_dir, filename), mode='RGB'),[299,299]).astype(np.float)
        image_ori = imresize(imread(os.path.join(origin_dir, filename), mode='RGB'),[299,299]).astype(np.float)
